    ...
    export default {
        ...
        methods: {
          ...
          getData() {
            ...
            if (this.searchClass == '小时') {
              ...
              api.GetCountyHourRank().then(res => {
                ...
                if (res.data.Status) {
                  ...
                  this.updateTime = allData[0].time;
                  let i = 1;
            ...
            }else if(this.searchClass == '日累计'){
              t.allPageData = [];
              api.GetCountyHourAccuRank().then(res => {
                // console.log(res)
                let allData = res.data.Data;
                this.updateTime = allData[0].time;
                if (res.data.Status) {
              ...
              t.allPageData = [];
              api.GetProportion(type).then(res=>{
                console.log(res)
                let allData = res.data.Data;
                if (res.data.Status) {
                  this.totalCount = allData.length;
          ...
          //导出
          ExportData(){
            if(this.searchClass=='小时'||this.searchClass=='日累计'){
              this.ExportCountyHourRank();
            }else{
              this.ExportProportion();
            }
          },
          // 导出
          ExportProportion(){
